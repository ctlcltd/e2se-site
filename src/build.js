...
class Build {
  ...
  watch(type) {
    ...
    fs.watch('./' + dst + '/' + type, { encoding: 'buffer' }, (eventType, file) => {
      if (eventType != 'change') return;

      fs.readFile('./' + dst + '/' + type + '/' + file, (err, data) => {
        if (type != 'scss') {
        ...
        if (type != 'scss') this.save(type, 100, true);
        else this.compile(type, true);
      });
    });
  }
  ...
  compile(type, no_uglify) {
    ...
    this.contents[type].forEach((filename, i) => {
      ...
      this['compile_' + type](filename, no_uglify);
    });
  }

  compile_scss(filename, no_uglify) {
    const name = 'style';
    const file = './' + dst + '/scss/' + filename + '.scss';
  ...
  concat(type) {
    ...
    if (this.options.js_banner) {
      js_banner = fs.readFileSync('./' + dst + '/js/_banner.js');
